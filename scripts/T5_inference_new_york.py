...
ny_df = pd.read_csv('/home/vivora/data/new_york.csv')

ny_df = ny_df[['plate','status']]

# Shuffle the entire dataset with a fixed random seed
ny_df = ny_df.sample(frac=1, random_state=42).reset_index(drop=True)

ny_df['plate'] = ny_df['plate'].astype(str)
ny_df['status'] = ny_df['status'].astype(str)
...
def process_vanity_plates(df, model, tokenizer, test_mode=True):
    ...
    if test_mode:
        ...
        output_filename = 'test_new_york_vanity_plates_meanings_T5_v2_rc.csv'
    else:
        # output_filename = 'cali_vanity_plates_meanings_T5_llama_rc.csv'
        output_filename = 'new_york_vanity_plates_meanings_T5_v2_rc.csv'
...
full_results = process_vanity_plates(ny_df, model, tokenizer, test_mode=False)
