...
class VanityPlateDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        comment = self.data.iloc[idx]["meaning"]
...
## Load CSV
data = pd.read_csv('../data/cali_v2_llama_rc.csv')
data = data.reset_index(drop=True)  
data["plate"] = data["plate"].fillna("").astype(str)
data["meaning"] = data["meaning"].fillna("").astype(str)
...
time_now = time_now + "_v2_llama_rc"
...
# Save to CSV
results_df.to_csv(f"../outputs/vanity_plate_predictions_{time_now}.csv", index=False)
logging.info(f"Results saved to 'vanity_plate_predictions_{time_now}.csv'.")
